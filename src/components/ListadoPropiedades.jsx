...
import {obtenerInmuebles, obtenerImagenPortada} from '../api/inmuebles';
...
const ListadoPropiedades = () => {
  ...
  const obtenerImagenPortadaInmueble = async (idInmueble) => {
    try {
      const imagenPortada = await obtenerImagenPortada(idInmueble);
    ...
    <div className='text-center'>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {Array.isArray(propiedades) && propiedades.length > 0 ? (
          propiedades.map(async (propiedad, index) => {
            const imagenPortada = await obtenerImagenPortadaInmueble(propiedad.id_propiedad);
