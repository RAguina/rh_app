...
export const obtenerComodidadesPorPropiedadId = async (propiedadId) => {
  try {
    const response = await axios.get(`${apiUrl}/${propiedadId}`);
...
export const crearComodidades = async (propiedadId, comodidades) => {
  try {
    const response = await axios.post(`${apiUrl}/${propiedadId}`, comodidades);
    return response.data;
  } catch (error) {
...
export const actualizarComodidades = async (propiedadId, comodidades) => {
  try {
    const response = await axios.put(`${apiUrl}/${propiedadId}`, comodidades);
    return response.data;
  } catch (error) {
